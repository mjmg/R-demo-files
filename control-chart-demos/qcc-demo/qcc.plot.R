                     ...
                     label.cl = waiver(), ...) 
{
  ...
  qc.gplot <- ggplotly(data = qc.data, environment = environment(), 
  ...
  qc.p3 <- ggplotly(qc.data, aes_string(x = df.indices, y = df.statistics), environment = environment()) +
    geom_blank() +
    theme_minimal() +
  ...
  if(chart.all & (!is.null(newstats))) { 
    ...
    qc.p2 <- ggplotly(qc.data, aes(x = df.indices, y = df.statistics), environment = environment()) +
      geom_blank() +
      theme_minimal() +
