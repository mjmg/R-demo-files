...
#First, let’s download the data from Kaggle and put it in the data/ folder in your working directory.
...
#In mxnet, we use the data type symbol to configure the network. data <- mx.symbol.Variable("data") uses data to represent the input data, i.e., the input layer.
#We set the first hidden layer with fc1 <- mx.symbol.FullyConnected(data, name="fc1", num_hidden=128). This layer has data as the input, its name, and the number of hidden neurons.
#Activation is set with act1 <- mx.symbol.Activation(fc1, name="relu1", act_type="relu"). The activation function takes the output from the first hidden layer, fc1.
#The second hidden layer takes the result from act1 as input, with its name as “fc2” and the number of hidden neurons as 64.
#The second activation is almost the same as act1, except we have a different input source and name.
#This generates the output layer. Because there are only 10 digits, we set the number of neurons to 10.
...
graph.viz(model$symbol, type = "viz", direction = "LR")  
